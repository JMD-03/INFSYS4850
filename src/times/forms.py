    ...
    def clean(self):
        ...
        if out_Time:
            if user is None:
                ...
                if 'in_Time2' in locals() and in_Time is None:
                ...
                if 'lunchout_Time2' in locals() and lunchout_Time is None:
                    if lunchout_Time2 and out_Time < lunchout_Time2:
                        raise forms.ValidationError("Your clock out time should be greater than saved lunch ending time. Please update your lunch start and end times to be within the boundaries of your new start/end times.")
                if 'lunchin_Time2' in locals() and lunchin_Time is None:
                    if lunchin_Time2  and out_Time < lunchin_Time2:
                        raise forms.ValidationError("Your clock out time should be greater than saved lunch start time. Please update your lunch start and end times to be within the boundaries of your new start/end times.")
                if 'in_Time2' in locals() and 'lunchin_Time2' in locals() and 'lunchout_Time2' in locals() and in_Time is None and out_Time is None:
            ...
            if in_Time and lunchin_Time and lunchout_Time:
                if (out_Time - (lunchout_Time - lunchin_Time) - in_Time).total_seconds() > 29700:
                    raise forms.ValidationError("Your clock out time cannot be greater than eight hours")
            if lunchout_Time  and out_Time < lunchout_Time:
                raise forms.ValidationError("Your clock out time should be greater than lunch end time")
            if lunchin_Time  and out_Time < lunchin_Time:
                raise forms.ValidationError("Your clock out time should be greater than lunch start time")
            if in_Time and lunchin_Time is None or lunchout_Time is None:
                if out_Time < in_Time:
                    raise forms.ValidationError("Your clock out time should be greater than your clock in time")
            if in_Time and not lunchin_Time and not lunchout_Time:
        ...
        if lunchout_Time:
            if user is None:
                ...
                if 'lunchin_Time2' not in locals() and lunchin_Time is None:
                    raise forms.ValidationError("you cannout end a lunch without starting one")
                if 'in_Time2' not in locals() and in_Time is None:
                    raise forms.ValidationError("you cannot end a lunch without clocking in")
                if 'lunchin_Time2' in locals() and lunchin_Time is None:
                    if lunchout_Time < lunchin_Time2:
                        raise forms.ValidationError("Your lunch end time should be greater than your saved lunch start")
                if 'in_Time2' in locals() and in_Time == None and lunchout_Time < in_Time2: #and in_Time is None:
            ...
            if lunchin_Time and lunchout_Time < lunchin_Time:
                raise forms.ValidationError("Your lunch end time should be greater than your lunch start")
            if in_Time and lunchout_Time < in_Time:
                raise forms.ValidationError("Your lunch end time should be greater than your clock in time")

        if lunchin_Time:
            if user is None:
                timecheck(lunchin_Time)
                if 'in_Time2' not in locals() and in_Time is None:
                    raise forms.ValidationError("you cannot start lunch without clocking in")
                if 'in_Time2' in locals() and in_Time == None and lunchin_Time < in_Time2: #and in_Time is None:
                    raise forms.ValidationError("your lunch start time should be greater than your saved clock in")
            if in_Time and lunchin_Time < in_Time:
                raise forms.ValidationError("Your lunch start time should be greater than your current clock in")
...
def timecheck(date):
    ...
    dater = timedelta(hours=6)
    tzNOW = timezone.now() - timedelta(hours=6)
    if date.day != tzNOW.day:
        raise forms.ValidationError("you can only clock in today" + "date.day: " + str(date.day) + "timezone.now: " + str(timezone.now()))
